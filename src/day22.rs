...
impl Field {
  fn parse(text: &str) -> Field {
    let bricks = text.lines().map(Brick::parse).collect::<Vec<_>>();
  ...
  fn view_x(&self) -> String {
    (1..=*self.range_z().end()).rev().fold(String::new(), |s, z| {
      s + &String::from_iter(self.range_x().map(|x| {
        as_char(self.bricks.iter().enumerate().filter_map(|(k, v)| {
          if v.cubes.iter().any(|p| p.0 == x && p.2 == z) {Some(k)} else {None}
        }).collect::<Vec<_>>())
      })) + "\n"
    })
  }
  fn view_y(&self) -> String {
    (1..=*self.range_z().end()).rev().fold(String::new(), |s, z| {
      s + &String::from_iter(self.range_y().map(|y| {
        as_char(self.bricks.iter().enumerate().filter_map(|(k, v)| {
          if v.cubes.iter().any(|p| p.1 == y && p.2 == z) {Some(k)} else {None}
        }).collect::<Vec<_>>())
      })) + "\n"
    })
  }
